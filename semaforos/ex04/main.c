    ...
    for(int i = 0; i < QUANT_PROCESSOS ; i++) {
		...
		if(pid == 0) {
            ...
            if(num > 0 && num < 30){
                ...
                timeStruct.tv_sec = 12;
                
                if(sem_timedwait(semWrite, &timeStruct) == -1) {
                    perror("Error at sem_wait().");
                    exit(3);
                }

                strcpy(infoPartilhada->data[infoPartilhada->quant_info], " ");
                infoPartilhada->quant_info--;

                printf("The function was able to delete the last written String.\n");
